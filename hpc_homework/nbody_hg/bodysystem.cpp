...
BodySystem::BodySystem(int numBodies)
: m_force(0),
...
void
BodySystem::_initialize(int numBodies)
{
...
void 
BodySystem::update(float deltaTime)
{
	assert(m_bInitialized);
...
float* 
BodySystem::getArray(BodyArray array)
{
	assert(m_bInitialized);
...
void 
BodySystem::setArray(BodyArray array, const float* data)
{
	assert(m_bInitialized);
...
void
BodySystem::_computeNBodyGravitation() 
{
...
void 
BodySystem::_integrateNBodySystem(float deltaTime)
{
...
bodyBodyInteraction(float accel[3], float posMass0[4], float posMass1[4], float softeningSquared) 
{
    ...
    float s = posMass1[3] * invDistCube; // why posMass1? instead of posMass0?
