...
static int puzzle_getview_from_cvimage(PuzzleContext * const context,
                                       PuzzleView * const view,
                                       IplImage* cvimage) {
    ...
    view->map = NULL;    
    view->width = (unsigned int) cvimage->width;
    view->height = (unsigned int) cvimage->height;
    ...
    do {
        ...
        do {
            CvScalar scalar;
            scalar = cvGet2D(cvimage, (int) y, (int) x);
            *maptr++ = (unsigned char)
                ((scalar.val[0] * 77 +
                  scalar.val[1] * 151 +
...
//    ret = puzzle_getview_from_gdimage(context, &view, gdimage);
    ret = puzzle_getview_from_cvimage(context, &view, cvimage);
