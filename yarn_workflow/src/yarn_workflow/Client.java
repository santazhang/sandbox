...
public class Client {
    ...
    // Folder containing the workflow specification
    ...
    public static final String WORKFLOW_PKG = "WORKFLOW.pkg";
    public static final String WORKFLOW = "WORKFLOW";
    public static final String SELF_EXTRACT_HEADER = "#!/bin/bash\n" +
    ...
    public Client(Configuration conf) throws Exception {
        this("yarn_workflow.ApplicationMaster", conf);
    }
    ...
    public boolean init(String[] args) throws ParseException {
        ...
        if (cliParser.hasOption("log_properties")) {
            ...
            try {
                yarn_workflow.Log4jPropertyHelper.updateLog4jConfiguration(Client.class, log4jPath);
            } catch (Exception e) {
        ...
        if (containerMemory < 0 || containerVirtualCores < 0) {
            throw new IllegalArgumentException("Invalid container memory/vcores specified, exiting."
    ...
    public boolean run() throws IOException, YarnException {
        ...
        Path workflowPkg = new Path(workflowDir + "/" + WORKFLOW_PKG);
        FileOutputStream pkgOut = new FileOutputStream(workflowPkg.toString());
        pkgOut.write(SELF_EXTRACT_HEADER.getBytes());

        DagHelper dagHelper = DagHelper.createFromFile(workflowDir + "/" + WORKFLOW);
        TarArchiveOutputStream tarOut
                = new TarArchiveOutputStream(new BufferedOutputStream(new GZIPOutputStream(pkgOut)));
        byte buffer[] = new byte[4096];
        for (String fname : dagHelper.listFileNames()) {
            File f = new File(workflowDir + "/" + fname);
            BufferedInputStream bi = new BufferedInputStream(new FileInputStream(f.getAbsolutePath()));
            TarArchiveEntry entry = new TarArchiveEntry(fname);
            entry.setSize(f.length());
            tarOut.putArchiveEntry(entry);
            int count;
            while ((count = bi.read(buffer, 0, buffer.length)) != -1) {
        ...
        String pathSuffix = appName + "/" + appId.toString() + "/" + WORKFLOW_PKG;
        Path workflowPkgHdfs = new Path(fs.getHomeDirectory(), pathSuffix);
        fs.copyFromLocalFile(false, true, workflowPkg, workflowPkgHdfs);

        payloadLocation += workflowPkgHdfs.toUri().toString();
        FileStatus fileStatus = fs.getFileStatus(workflowPkgHdfs);
        payloadTimestamp += Long.toString(fileStatus.getModificationTime());
        payloadSize += Long.toString(fileStatus.getLen());
        ...
        env.put(Constants.YARNWORKFLOW_PAYLOADLOCATION, payloadLocation);
        env.put(Constants.YARNWORKFLOW_PAYLOADTIMESTAMP, payloadTimestamp);
        env.put(Constants.YARNWORKFLOW_PAYLOADSIZE, payloadSize);
