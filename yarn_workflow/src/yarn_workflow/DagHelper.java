...
public class DagHelper {
    ...
    public static DagHelper createFromFile(String fpath) {
        try {
            File f = new File(fpath);
            byte[] bytes = Files.readAllBytes(f.toPath());
            DagHelper ret = createFromString(new String(bytes,"UTF-8"));
            ret.filenames.add(f.getName());
            return ret;
